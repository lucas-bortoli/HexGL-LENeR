...
bkcore.hexgl.HexGL = function (opts) {
  ...
  this.track =
    bkcore.hexgl.tracks[opts.track == undefined ? "Cityscape" : opts.track];
...
bkcore.hexgl.HexGL.prototype.displayScore = function (f, l) {
  ...
  if (this.gameplay.result == this.gameplay.results.FINISH) {
    ...
    if (typeof Storage !== "undefined") {
      if (
        localStorage["score-" + t + "-" + d] == undefined ||
        ...
        // Export race data
        localStorage["race-" + t + "-replay"] = JSON.Stringify(
  ...
  else sl != undefined && (sl.innerHTML = "");
